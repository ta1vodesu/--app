...
| 2026-07-14 | 全体デバッグ完了: 月末日計算バグ・JST日付ズレ・承認の勤怠未反映・出勤率/残業の計算式を修正。デッドコード15ファイル削除。ロールは admin/member の2種で確定。ステータスは打刻実績ベースで正規化（working/worked混在をUI側で吸収）。モバイルはドロワー方式サイドバーのみ（ボトムナビ廃止）。migration 007（status制約統一）は未適用 |
